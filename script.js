...
let startGame = () => {
  ...
  for (let i = 0; i < GRID_LEN; i++) grid[i] = new Array(GRID_LEN);
  ...
  cells.forEach((cell) => {
    ...
    // placing bomb
    if (bombsIndices.has(+cell.id - 1)) grid[r][c].isBomb = true;
  });
...
let countAdjacentBombs = (cell) => {
  ...
  for (let i = -1; i <= 1; i++) {
    for (let j = -1; j <= 1; j++) {
      if (
        validPos(cell.row + i, cell.col + j) &&
        grid[cell.row + i][cell.col + j].isBomb
      )
        count += 1;
    }
  }
...
let cellClick = (event) => {
  ...
  // if bomb is clicked
  if (grid[r][c].isBomb) gameOver(event.target);
  else {
    floodFill(r, c);
    checkWin();
  }
...
let checkWin = () => {
  if (cellsOpened !== GRID_LEN * GRID_LEN - BOMBS) return;
